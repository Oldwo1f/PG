...
const openCreateExampleModal = () => {
	// Préremplir les champs avec les informations du template sélectionné
	if (selectedTemplate.value) {
		// S'assurer que la catégorie est une string valide
		const categoryValue = selectedTemplate.value.category 
			? String(selectedTemplate.value.category) 
			: "";
		...
		exampleTemplate.value = {
			...
			previewImage: selectedTemplate.value.previewImage || "",
			layout: {
				width: selectedTemplate.value.layout?.width || 1000,
				height: selectedTemplate.value.layout?.height || 600,
			},
			tagsInput: selectedTemplate.value.tags?.join(", ") || "",
		};
	} else {
		// Réinitialiser si aucun template n'est sélectionné
		exampleTemplate.value = {
			name: "",
			description: "",
			category: "",
	...
	showCreateExampleModal.value = true;
};
...
// Watcher pour déclencher automatiquement la génération de preview quand le nom est renseigné
let previewGenerationTimeout: ReturnType<typeof setTimeout> | null = null;
let hasAutoGeneratedPreview = false; // Flag pour éviter de régénérer si déjà fait
watch(
	() => exampleTemplate.value.name,
	(newName, oldName) => {
		// Ne déclencher que si :
		// 1. Le nom est renseigné (non vide après trim)
		// 2. Le modal est ouvert
		// 3. La génération n'est pas déjà en cours
		// 4. Le nom a changé (pas juste au montage)
		// 5. Le nom passe de vide à non-vide (première fois qu'il est renseigné)
		const wasEmpty = !oldName?.trim();
		const isNowFilled = !!newName?.trim();
		
		if (
			isNowFilled &&
			wasEmpty &&
			showCreateExampleModal.value &&
			!generatingExamplePreview.value &&
			!hasAutoGeneratedPreview &&
			oldName !== undefined
		) {
			// Debounce pour éviter de déclencher à chaque frappe
			if (previewGenerationTimeout) {
				clearTimeout(previewGenerationTimeout);
			}
			previewGenerationTimeout = setTimeout(() => {
				hasAutoGeneratedPreview = true;
...
watch(showCreateExampleModal, (isOpen) => {
	if (!isOpen) {
		hasAutoGeneratedPreview = false;
		if (previewGenerationTimeout) {
			clearTimeout(previewGenerationTimeout);
